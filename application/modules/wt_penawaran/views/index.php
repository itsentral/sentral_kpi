	...
	<div class="box-body">
		...
		<tr>
			...
			<?php if($ENABLE_MANAGE AND $record->status == '2') : ?>
				<a class="btn btn-primary btn-sm" href="<?= base_url('/wt_penawaran/editpenawaranapprove/'.$record->no_penawaran) ?>" title="Edit" data-no_inquiry="<?=$record->no_inquiry?>"><i class="fa fa-edit"></i>
				</a>
			<?php endif; ?>		
			<?php endif; ?>	
			...
			<?php if($record->status != '4') : ?>			
			<?php if($ENABLE_MANAGE AND $record->status != '2') : ?>
				<a class="btn btn-primary btn-sm" href="<?= base_url('/wt_penawaran/editpenawaran/'.$record->no_penawaran) ?>" title="Edit" data-no_inquiry="<?=$record->no_inquiry?>"><i class="fa fa-edit"></i>
				</a>
			<?php endif; ?>		
			<?php endif; ?>		
			
			<?php if($ENABLE_VIEW AND $record->status != '4') : ?>
				<a class="btn btn-info btn-sm" href="<?= base_url('/wt_penawaran/printpenawaran/'.$record->no_penawaran) ?>" target="_blank" title="Print" data-no_inquiry="<?=$record->no_inquiry?>"><i class="fa fa-print"></i>
				</a>
			<?php endif; ?>

			<?php if($ENABLE_MANAGE AND $record->status != '4' AND $record->printed_by != NULL) : ?>
				<a class="btn btn-success btn-sm" href="<?= base_url('/wt_penawaran/statusterkirim/'.$record->no_penawaran) ?>" title="Ubah Status" data-no_inquiry="<?=$record->no_inquiry?>"><i class="fa fa-check"></i>
				</a>
			<?php endif; ?>

			<?php if($ENABLE_MANAGE AND $record->status != '4') : ?>
				<a class="btn btn-warning btn-sm" href="<?= base_url('/wt_penawaran/ajukanapprove/'.$record->no_penawaran) ?>" title="Ajukan approval" data-no_inquiry="<?=$record->no_inquiry?>"><i class="fa fa-mail-forward"></i>
				</a>
			<?php endif; ?>
			

			<?php if($ENABLE_MANAGE AND $record->status == '4') : ?>
				<a class="btn btn-primary btn-sm" href="<?= base_url('/wt_penawaran/revisipenawaran/'.$record->no_penawaran) ?>" title="Revisi" data-no_inquiry="<?=$record->no_inquiry?>"><i class="fa fa-list"></i>
				</a>
			<?php endif; ?>
			<?php if($ENABLE_MANAGE AND $record->status == '4' AND $record->printed_by != NULL) : ?>
			<a class="btn btn-success btn-sm" href="<?= base_url('/wt_sales_order/createSO/'.$record->no_penawaran) ?>" title="Create SO" data-no_inquiry="<?=$record->no_inquiry?>"><i class="fa fa-plus">Create SO</i>
			</a> 
			<?php endif; ?>
			<?php if($ENABLE_MANAGE AND $record->status == '4') : ?>
				<a class="btn btn-danger btn-sm" href="<?= base_url('/wt_penawaran/statusloss/'.$record->no_penawaran) ?>" title="Loss" data-no_inquiry="<?=$record->no_inquiry?>"><i class="fa fa-check"></i>
				</a>
			<?php endif; ?>
