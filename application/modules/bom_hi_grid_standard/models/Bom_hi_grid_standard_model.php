...
class Bom_hi_grid_standard_model extends BF_Model{

    public function __construct()
    {
        parent::__construct();

		$this->ENABLE_ADD     = has_permission('BOM_HI-Grid_Standard.Add');
		$this->ENABLE_MANAGE  = has_permission('BOM_HI-Grid_Standard.Manage');
		$this->ENABLE_VIEW    = has_permission('BOM_HI-Grid_Standard.View');
		$this->ENABLE_DELETE  = has_permission('BOM_HI-Grid_Standard.Delete');
    }
	...
	public function get_data_where_array($table,$where){
  	...
  	public function get_json_bom(){
		...
		foreach($query->result_array() as $row){
			...
			$nestedData[]	= "<div align='left'>".strtoupper(strtolower($row['surface']))."</div>";
	...
	public function get_query_json_bom($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		$sql = "SELECT
				...
				FROM
					...
					(SELECT @row:=0) r
				WHERE 1=1 AND a.deleted_date IS NULL AND a.category = 'grid standard' AND 
					(
