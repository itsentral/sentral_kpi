...
class Stok_subgudang_model extends BF_Model{
  ...
  public function get_json_stock(){
		...
		$urut1  = 1;
		$urut2  = 0;
		$id_gudang = $requestData['id_gudang'];
		...
		$nm_warehouse = (!empty($GET_WAREHOUSE[$id_gudang]['nm_gudang']))?$GET_WAREHOUSE[$id_gudang]['nm_gudang']:'';
		foreach($query->result_array() as $row){
			...
			$nestedData[]	= "<div align='left'>".strtoupper($nm_warehouse)."</div>";
	...
	public function get_query_json_stock($date_filter, $id_gudang, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		$sql = "SELECT
            ...
            FROM
              new_inventory_4 a,
              (SELECT @row:=0) r
            WHERE a.deleted_date IS NULL AND a.category='material' AND (
