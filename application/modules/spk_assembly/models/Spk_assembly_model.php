...
class Spk_assembly_model extends BF_Model{

  public function __construct(){
    parent::__construct();

    $this->ENABLE_ADD     = has_permission('SPK_Assembly.Add');
		$this->ENABLE_MANAGE  = has_permission('SPK_Assembly.Manage');
		$this->ENABLE_VIEW    = has_permission('SPK_Assembly.View');
		$this->ENABLE_DELETE  = has_permission('SPK_Assembly.Delete');
  ...
  public function data_side_request_produksi(){
    ...
    foreach($query->result_array() as $row){
      ...
      if($this->ENABLE_MANAGE){
        $edit	= "<a href='".site_url($this->uri->segment(1)).'/add/'.$row['id']."' class='btn btn-sm btn-primary' title='Request' data-role='qtip'><i class='fa fa-hand-pointer-o'></i></a>";
      }
  ...
  public function get_query_request_produksi($product, $costcenter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
            ...
            FROM
                so_spk_assembly a
                LEFT JOIN so_internal_request y ON a.so_number = y.so_number
                LEFT JOIN customer c ON a.id_customer=c.id_customer,
                (SELECT @row:=0) r
            WHERE 1=1 ".$costcenter_where." ".$product_where." AND (
               y.nama_product LIKE '%".$this->db->escape_like_str($like_value)."%'
