...
class Produksi_request_list_cutting_model extends BF_Model{

  public function __construct(){
    parent::__construct();

    $this->ENABLE_ADD     = has_permission('Request_List_Produksi_Cutting.Add');
		$this->ENABLE_MANAGE  = has_permission('Request_List_Produksi_Cutting.Manage');
		$this->ENABLE_VIEW    = has_permission('Request_List_Produksi_Cutting.View');
		$this->ENABLE_DELETE  = has_permission('Request_List_Produksi_Cutting.Delete');
  }
  ...
  public function data_side_request_material(){
    ...
    foreach($query->result_array() as $row){
      ...
      if($row['sts_confirm'] == 'N'){
        $print	= "&nbsp;<a href='".base_url('produksi_request_list_cutting/print_spk_request/'.$row['kode_trans'])."' target='_blank' class='btn btn-sm btn-info' title='Print SPK Permintaan Material' data-role='qtip'><i class='fa fa-print'></i></a>";
      }
  ...
  public function get_query_json_request_material($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
              ...
              e.variant_product,
              e.color AS color_product,
              e.surface AS surface_product
            FROM
              warehouse_adjustment a
              LEFT JOIN so_spk_cutting_request x ON a.no_ipp = x.id
              LEFT JOIN so_spk_cutting y ON x.id_so = y.id
              LEFT JOIN new_inventory_4 z ON y.code_lv4 = z.code_lv4
              LEFT JOIN ms_costcenter c ON a.kd_gudang_ke=c.id_costcenter
              LEFT JOIN warehouse d ON a.id_gudang_dari=d.id
              LEFT JOIN bom_header e ON y.no_bom=e.no_bom,
              (SELECT @row:=0) r
            WHERE a.deleted_date IS NULL AND a.category='request produksi cutting' AND (
              a.kode_trans LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR d.nm_gudang LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR c.nama_costcenter LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR x.no_spk LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR z.nama LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR e.variant_product LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR e.color LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR e.surface LIKE '%".$this->db->escape_like_str($like_value)."%'
            )
            ";
    // echo $sql; exit;
