...
class Slitting_cost_model extends BF_Model
{
    ...
    function generate_id() {
      $query = $this->db->query("SELECT MAX(id_supplier) as max_id FROM master_supplier");
      $row = $query->row_array();
      $max_id = $row['max_id'];
      $max_id1 =(int) substr($max_id,1,4);
      $counter = $max_id1 +1;
      $idsup = "S".str_pad($counter, 4, "0", STR_PAD_LEFT);
      return $idsup;
    }
	...
	function getById($id){
		...
		return $supplier = $query->row_array();
	}
