 ...
 *
 */
class Bom_topping extends Admin_Controller
{
    //Permission
    protected $viewPermission 	= 'BOM_Topping.View';
    protected $addPermission  	= 'BOM_Topping.Add';
    protected $managePermission = 'BOM_Topping.Manage';
    protected $deletePermission = 'BOM_Topping.Delete';
   ...
   public function __construct()
    {
        ...
        $this->load->model(array('Bom_topping/bom_topping_model'
                                ));
    ...
    public function index(){
      ...
      $deleted = '0';
      $data = $this->bom_topping_model->get_data('bom_header','deleted','N');
      history("View index BOM Topping");
      $this->template->set('results', $data);
      $this->template->title('BOM Topping');
      $this->template->render('index');
    }

    public function data_side_bom(){
      $this->bom_topping_model->get_json_bom();
    }
    ...
    public function add(){
      if($this->input->post()){
        ...
        // if($num_p < 1){
          ...
          if(empty($no_bomx)){
            //pengurutan kode
            $srcMtr			  = "SELECT MAX(no_bom) as maxP FROM bom_header WHERE no_bom LIKE 'BOT".$Ym."%' ";
        ...
        $detail   = $this->db->get_where('bom_detail',array('no_bom' => $no_bom))->result_array();
  		$product    = $this->bom_topping_model->get_data_where_array('new_inventory_3',array('deleted_date'=>NULL,'category'=>'product'));
  		$product_4    = $this->bom_topping_model->get_data_where_array('new_inventory_4',array('deleted_date'=>NULL,'category'=>'product'));
  		$product_3    = $this->bom_topping_model->get_data_where_array('new_inventory_3',array('deleted_date'=>NULL,'category'=>'product'));
        $material    = $this->bom_topping_model->get_data_where_array('new_inventory_4',array('deleted_date'=>NULL,'category'=>'material'));
        ...
        // exit;
          ...
          'material' => $material
  			...
  			$this->template->set('results', $data);
        $this->template->title('Add BOM Topping');
    ...
    public function excel_report_all_bom(){
    	...
    	$sheet 		= $objPHPExcel->getActiveSheet();
  		...
  		$product    = $this->db
						...
						->join('new_inventory_3 b','a.id_product=b.code_lv3','left')
						->get_where('bom_header a',array('a.deleted_date'=>NULL,'a.category'=>'topping'))
						->result_array();

		$Row		= 1;
		$NewRow		= $Row+1;
		$Col_Akhir	= $Cols	= getColsChar(6);
		$sheet->setCellValue('A'.$Row, 'BOM TOPPING');
		...
		$sheet->setCellValue('C'.$NewRow, 'Jenis Topping');
		...
		$sheet->getColumnDimension('F')->setAutoSize(true);
  		...
  		header('Content-Disposition: attachment;filename="bom-topping.xls"');
  		//unduh file
    ...
    public function excel_report_all_bom_detail(){
    	...
    	$sheet 		= $objPHPExcel->getActiveSheet();
  		...
  		$sql = "
  			...
  			FROM
				...
				LEFT JOIN new_inventory_3 c ON a.id_product = c.code_lv3
  		    WHERE 
				a.no_bom = '".$kode_bom."' 
				AND b.no_bom = '".$kode_bom."'
				AND a.category = 'topping'
  			ORDER BY
  				b.id ASC
  		";
		...
		$sheet->setCellValue('A'.$Row, 'BOM TOPPING DETAIL');
		$sheet->getStyle('A'.$Row.':'.$Col_Akhir.$NewRow)->applyFromArray($mainTitle);
		$sheet->mergeCells('A'.$Row.':'.$Col_Akhir.$NewRow);
		...
		$sheet->getColumnDimension('C')->setAutoSize(true);
  		...
  		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  		//ubah nama file saat diunduh
  		header('Content-Disposition: attachment;filename="bom-topping-detail-'.$kode_bom.'.xls"');
  		//unduh file
  		$objWriter->save("php://output");
  	}
