...
class Production_report_ftackle_model extends BF_Model
{
  ...
  public function data_side_spk_material()
  {
    ...
    foreach ($query->result_array() as $row) {
      ...
      $tgl_close  = (!empty($row['tanggal_close_new'])) ? date('d-M-Y', strtotime($row['tanggal_close_new'])) : '';
      $est_finish = (!empty($row['est_finish_date'])) ? date('d-M-Y', strtotime($row['est_finish_date'])) : '';
      ...
      $nestedData[]  = "<div align='center'>" . $est_finish . "</div>";
      $nestedData[]  = "<div align='center'>" . $tgl_close . "</div>";
      ...
      $sts_produksi = "<span class='badge bg-blue'>Parsial</span>";
      if ($row['sts_close'] == 'Y') {
        $sts_produksi = "<span class='badge bg-green'>Close</span>";
      }

      $nestedData[]  = "<div align='left'>" . $sts_produksi . "</div>";
      $nestedData[]  = "<div align='center'><a href='" . base_url('production_report_ftackle/view_process/' . $row['id']) . "' class='btn btn-sm btn-primary' data-id='" . $row['id'] . "'>View Process</a></div>";
      $data[] = $nestedData;
      $urut1++;
      $urut2++;
    }
