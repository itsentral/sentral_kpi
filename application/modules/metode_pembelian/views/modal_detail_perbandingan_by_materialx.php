...
foreach($dt_supplier AS $val => $valx){
    $jumlah_supp++;
    $header_supp.='<th class="text-center">'.$valx->nm_supplier.'</th>';
}
$no  = 0;
    ...
    <table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
        ...
        <tbody>
            ...
            foreach($datarow as $keys => $value) {
                ...
                <td align=center>'.$value['price_ref'].'</td>
                <td align=center>'.$value['qty'].'</td>';
                foreach($dt_supplier AS $val => $valx){
                    echo '<td align=center nowrap><span class="text-primary text-bold">'.strtoupper($value[$valx->id_supplier]['currency']).'</span> '.number_format($value[$valx->id_supplier]['price_ref_sup']).'</td>';
                }
                foreach($dt_supplier AS $val => $valx){
