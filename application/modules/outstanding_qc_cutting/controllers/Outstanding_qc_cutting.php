...
class Outstanding_qc_cutting extends Admin_Controller
{
  //Permission
  protected $viewPermission   = 'Outstanding_QC_Cutting.View';
  protected $addPermission    = 'Outstanding_QC_Cutting.Add';
  protected $managePermission = 'Outstanding_QC_Cutting.Manage';
  protected $deletePermission = 'Outstanding_QC_Cutting.Delete';
  ...
  public function __construct()
  {
    ...
    $this->load->model(array(
      'Outstanding_qc_cutting/Outstanding_qc_cutting_model'
    ));
  ...
  public function index()
  {
    ...
    $this->template->title('Quality Control / Outstanding Cutting QC');
    $this->template->render('index');
  }

  public function data_side_outstanding_qc()
  {
    $this->Outstanding_qc_cutting_model->data_side_outstanding_qc();
  }
  ...
  public function qc($id = null, $id_cut_plan = null)
  {
    ...
    if ($this->input->post()) {
      ...
      $this->db->trans_start();
      if (!empty($UpdateData)) {
        $this->db->insert_batch('so_spk_cutting_product', $UpdateData);
      ...
      $listData = $this->db
        ...
        ->get_where('so_spk_cutting_product z', array('z.id_key_spk' => $id_cut_plan, 'z.kode_det' => $id))
        ->result_array();
  ...
  public function qcNew()
  {
    ...
    $this->db->trans_start();
    if (!empty($UpdateData)) {
      $this->db->update_batch('so_spk_cutting_product', $UpdateData, 'id');
