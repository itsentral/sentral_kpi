...
class Outstanding_qc_cutting_model extends BF_Model{

  public function __construct(){
    parent::__construct();

    $this->ENABLE_ADD     = has_permission('Outstanding_QC_Cutting.Add');
		$this->ENABLE_MANAGE  = has_permission('Outstanding_QC_Cutting.Manage');
		$this->ENABLE_VIEW    = has_permission('Outstanding_QC_Cutting.View');
		$this->ENABLE_DELETE  = has_permission('Outstanding_QC_Cutting.Delete');
  ...
  public function data_side_outstanding_qc(){
    ...
    foreach($query->result_array() as $row){
      ...
      $QC_SISA = $QTY;
      $QC_ALL = $row['qty'];
      // if($row['subgudang_date'] >= '2024-02-25 00:00:00'){
        $getQC = $this->db->get_where('so_spk_cutting_product',array('id_key_spk'=>$row['id_cut_plan'],'kode_det'=>$row['id'],'status'=>'N'))->result();
      ...
      if($QC_SISA > 0){
        ...
        $approve	= "<button type='button' data-id='".$row['id']."' data-id_cut_plan='".$row['id_cut_plan']."' class='btn btn-sm btn-success detail' title='QC' data-role='qtip'><i class='fa fa-check'></i></a>";
        }
  ...
  public function get_query_outstanding_qc($product, $costcenter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
              ...
              c.nm_customer,
              a.project,
              z.outgoing_date AS close_produksi,
              ...
              p.id as id_cut_plan
            FROM
                so_spk_cutting_product y
                LEFT JOIN so_spk_cutting_request z ON z.id = y.kode_det
                LEFT JOIN so_spk_cutting a ON z.id_so = a.id AND z.status_id = '1'
                LEFT JOIN so_spk_cutting_plan p ON y.id_key_spk = p.id
                ...
                (SELECT @row:=0) r
            WHERE a.deleted_date IS NULL ".$costcenter_where." ".$product_where." AND (
            ...
            GROUP BY y.kode_det, y.id_key_spk
            ";
    // echo $sql; exit;
