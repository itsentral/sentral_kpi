...
class Request_new_product extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('Request_new_product/Request_new_product_model');
		$this->template->title('Request_new_product');
	...
	public function print_quotation($no_penawaran)
	{
		...
		$this->template->title('Print Request_new_product');
		$this->template->render('print_quotation');
	}

	public function server_side_inv()
	{
		$this->Request_new_product_model->get_data_json_inv();
	}
	...
	{
		$this->Request_new_product_model->get_data_json_payment();
	}
	...
	{
		$this->Request_new_product_model->modal_detail_invoice($no_penawaran);
	}

	public function approval_quotation($no_penawaran)
	{
		$this->Request_new_product_model->approval_quotation($no_penawaran);
	}

	public function view_quotation($no_penawaran)
	{
		$this->Request_new_product_model->view_quotation($no_penawaran);
	}

	public function modal_detail_invoice_np()
	{
		$this->Request_new_product_model->modal_detail_invoice_np($this->uri->segment(3));
	}
	...
	public function save_penerimaan()
	{
		...
		if ($no_surat == '') {
			$no_penawaran = $this->Request_new_product_model->generate_no_penawaran();
			...
			if (isset($post['select_action'])) {
				...
				if ($get_penawaran->req_app1 !== null || $get_penawaran->req_app2 !== null || $get_penawaran->req_app3 !== null) {
					...
					$this->db->insert('tr_history_penawaran', [
						'id_history_penawaran' => $this->Request_new_product_model->generate_id_history(),
	...
	public function jurnal_bum()
	{
		...
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-list');
		$data = $this->Request_new_product_model->get_data_pn_jurnal();
		$this->template->set('results', $data);
	...
	public function save_penerimaan_np()
	{
		...
		$data_session 	    = $this->session->userdata;
		$kd_bayar 			= $this->Request_new_product_model->generate_nopn_np($Tgl_Invoice);
	...
	public function jurnal_bum_np()
	{
		...
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-list');
		$data = $this->Request_new_product_model->get_data_pn_jurnal_np();
		$this->template->set('results', $data);
		$this->template->title('Jurnal Penerimaan');
	...
	{
		$data = $this->Request_new_product_model->get_json_product_price();
		// print_r($data);
		// exit;
	...
	public function createunlocated()
	{
		if ($this->input->post()) {
			...
			$customer   	= $this->db->order_by('name_customer', 'asc')->get_where('master_customer', array('deleted_date' => NULL))->result_array();
	...
	{
		$this->Request_new_product_model->get_json_ipp();
	}
