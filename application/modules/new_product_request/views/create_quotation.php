							...
							if (isset($results['data_penawaran_detail'])) {
								foreach ($results['data_penawaran_detail'] as $penawaran_detail) {
									...
									<tr>
										...
										<td>
											<input type="text" name="diskon_persen_' . $penawaran_detail->id_penawaran_detail . '" id="" class="form-control diskon_persen diskon_persen_' . $penawaran_detail->id_penawaran_detail . '" placeholder="Input (%)" value="' . $penawaran_detail->diskon_persen . '%" onchange="hitung_all(' . $penawaran_detail->id_penawaran_detail . ')">
						...
						<div class="row">
							...
							<div class="col-lg-6">
								<div class="form-group " style="padding-top:15px;">
									<label class="col-sm-4 control-label">Total price before discount</label>
									<div class="col-sm-6">
							...
							<div class="col-lg-6">
								<div class="form-group " style="padding-top:15px;">
									<label class="col-sm-4 control-label">Total price after discount</label>
									<div class="col-sm-6">
										<input type="text" name="total" class="form-control input-sm text-right" id="total" value="<?= number_format($total_all) ?>" readonly tabindex="-1">
									</div>
								</div>
							</div>
							<div class="col-lg-6"></div>
							<div class="col-lg-6">
								<div class="form-group " style="padding-top:15px;">
									...
									if (isset($results['data_penawaran'])) {
										$grand_total = ($total_all + ($results['data_penawaran']->nilai_ppn));
									}
									?>
									...
									<div class="col-sm-3">
										<input type="text" name="nilai_ppn" class="form-control input-sm text-right nilai_ppn" id="total" value="<?= (isset($results['data_penawaran'])) ? ($results['data_penawaran']->nilai_ppn) : null ?>">
									</div>
								</div>
							</div>
							<div class="col-lg-6"></div>
						...
						<div class="row">
							<div class="col-lg-12">
								<button id="simpanpenerimaan" class="btn btn-primary" type="button" onclick="savemutasi()">
									<i class="fa fa-save"></i><b> Simpan Data Penawaran</b>
								</button>
			...
			<div class="modal modal-default fade" id="sales_product_price_list_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				<div class="modal-dialog modal-lg">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Tutup</span></button>
							<h4 class="modal-title title_modal" id="myModalLabel"><span class="fa fa-users"></span>&nbsp;List Invoice</h4>
						</div>
			...
			<script>
				...
				*/
					...
					$.ajax({
						...
						success: function(result) {
							...
							if (result.status == '1') {
								...
								$('.select_product_price_' + id).attr('disabled', false);
							}
				...
				$(document).on('change', '.persen_ppn', function() {
					...
					ppn_persen = ppn_persen.split(',').join('');
					ppn_persen = ppn_persen.split('%').join('');
					ppn_persen = parseFloat(ppn_persen);
