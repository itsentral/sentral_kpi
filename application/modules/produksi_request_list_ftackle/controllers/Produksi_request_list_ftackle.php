...
class Produksi_request_list_ftackle extends Admin_Controller
{
    //Permission
    protected $viewPermission 	= 'Request_List_Produksi_F-Tackle.View';
    protected $addPermission  	= 'Request_List_Produksi_F-Tackle.Add';
    protected $managePermission = 'Request_List_Produksi_F-Tackle.Manage';
    protected $deletePermission = 'Request_List_Produksi_F-Tackle.Delete';
   ...
   public function __construct()
    {
        ...
        $this->load->model(array('Produksi_request_list_ftackle/produksi_request_list_ftackle_model'
                                ));
    ...
    public function index(){
      ...
      $this->template->title('Gudang Material / Gudang Produksi / Request List F-Tackle');
      $this->template->render('index');
    }

    public function data_side_spk_material(){
  		$this->produksi_request_list_ftackle_model->data_side_spk_material();
  	}
    ...
    public function request($id=null){
      if($this->input->post()){
        ...
        if(!empty($data['detail'])){
          foreach($detail AS $val => $valx){
            ...
            if($qty_packing > 0){
              ...
              if(!empty($valx['id'])){
                $getRequest = $this->db->get_where('so_internal_spk_material',array('id'=>$valx['id']))->result_array();
                ...
                $getCodeSPK = $this->db->select('kode_det')->get_where('so_internal_spk',array('id'=>$id_spk))->result_array();
        ...
        $ArrInsert = array(
          ...
          'no_ipp' 		      => $id_spk,
          'category' 			  => 'request produksi ftackle',
          'jumlah_mat' 		      => $SUM_MAT,
        ...
        $listSO         = $this->db->select('a.*')->join('new_inventory_4 b','a.code_lv4=b.code_lv4')->get_where('so_internal a',array('b.code_lv1'=>'P123000009'))->result_array();
        $listGudang     = $this->db->where_in('desc',array('subgudang','pusat'))->get_where('warehouse',array('id'=>16))->result_array();
        $listCostcenter = $this->db->select('a.id AS id_costcenter, b.nama_costcenter')->join('ms_costcenter b','a.kd_gudang=b.id_costcenter')->get_where('warehouse a',array('a.desc'=>'costcenter','a.id'=>6))->result_array();
    ...
    public function server_side_request_produksi(){
      $this->produksi_request_list_ftackle_model->server_side_request_produksi();
    }

    public function data_side_request_material(){
  		$this->produksi_request_list_ftackle_model->data_side_request_material();
  	}
    ...
    public function print_spk_request(){
      ...
      $getData = $this->db
                            ...
                            ->join('so_internal c','c.id=b.id_so','left')
    ...
    public function get_list_spk(){
      ...
      $result	= $this->db->where_not_in('Y')->get_where('so_internal_spk',array('id_so'=>$id_so))->result_array();
    ...
    public function get_estimasi_material(){
      ...
      $result	= $this->db
                      ...
                      ->get_where('so_internal_spk_material a',array('a.kode_det'=>$kode_det,'a.type_name !='=>'mixing'))->result_array();


      //Stok SPK
      $id = get_name('so_internal_spk','id','kode_det',$kode_det);
      $QUERY = "SELECT
                  a.id_material,
