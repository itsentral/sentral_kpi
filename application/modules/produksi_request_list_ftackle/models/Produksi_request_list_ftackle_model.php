...
class Produksi_request_list_ftackle_model extends BF_Model{

  public function __construct(){
    parent::__construct();

    $this->ENABLE_ADD     = has_permission('Request_List_Produksi_F-Tackle.Add');
		$this->ENABLE_MANAGE  = has_permission('Request_List_Produksi_F-Tackle.Manage');
		$this->ENABLE_VIEW    = has_permission('Request_List_Produksi_F-Tackle.View');
		$this->ENABLE_DELETE  = has_permission('Request_List_Produksi_F-Tackle.Delete');
  }
  ...
  public function data_side_request_material(){
    ...
    foreach($query->result_array() as $row){
      ...
      if($row['sts_confirm'] == 'N'){
        $print	= "&nbsp;<a href='".base_url('produksi_request_list_ftackle/print_spk_request/'.$row['kode_trans'])."' target='_blank' class='btn btn-sm btn-info' title='Print SPK Permintaan Material' data-role='qtip'><i class='fa fa-print'></i></a>";
      }
  ...
  public function get_query_json_request_material($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              LEFT JOIN so_internal_spk x ON a.no_ipp = x.id
              LEFT JOIN so_internal y ON x.id_so = y.id
              ...
              (SELECT @row:=0) r
            WHERE a.deleted_date IS NULL AND a.category='request produksi ftackle' AND (
