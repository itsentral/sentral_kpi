...
class Bom_ftackel_model extends BF_Model{

    public function __construct()
    {
        parent::__construct();

		$this->ENABLE_ADD     = has_permission('BOM_F-Tackel.Add');
		$this->ENABLE_MANAGE  = has_permission('BOM_F-Tackel.Manage');
		$this->ENABLE_VIEW    = has_permission('BOM_F-Tackel.View');
		$this->ENABLE_DELETE  = has_permission('BOM_F-Tackel.Delete');
    }
	...
	public function get_data_where_array($table,$where){
  	...
  	public function get_json_bom(){
		...
		foreach($query->result_array() as $row){
        	...
        	$berat = $this->db->query("SELECT SUM(weight) AS weight FROM bom_detail WHERE no_bom='".$row['no_bom']."' and category = 'default'")->result();
	...
	public function get_query_json_bom($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		$sql = "SELECT
				...
				FROM
					...
					(SELECT @row:=0) r
				WHERE 1=1 AND a.deleted_date IS NULL AND a.category = 'ftackel' AND 
					(
