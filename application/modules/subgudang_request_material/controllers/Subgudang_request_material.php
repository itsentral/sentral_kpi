 ...
 *
 */
class Subgudang_request_material extends Admin_Controller
{
    //Permission
    protected $viewPermission 	= 'Request_Material_Subgudang.View';
    protected $addPermission  	= 'Request_Material_Subgudang.Add';
    protected $managePermission = 'Request_Material_Subgudang.Manage';
    protected $deletePermission = 'Request_Material_Subgudang.Delete';
   ...
   public function __construct()
    {
        ...
        $this->load->model(array('Subgudang_request_material/subgudang_request_material_model'
                                ));
    ...
    public function index(){
      ...
      history("View data subgudang request");
      $this->template->title('Gudang Material / Subgudang / Request Material');
      $this->template->render('index');
    }

    public function data_side_request_material(){
  		$this->subgudang_request_material_model->data_side_request_material();
  	}
    ...
    public function request($id=null){
      if($this->input->post()){
        ...
        if($this->db->trans_status() === FALSE){
          ...
          history("Request subgudang to gudang pusat : ".$kode_trans);
        }
        echo json_encode($Arr_Data);
      }
      else{

        $listGudang     = $this->db->get_where('warehouse',array('desc'=>'pusat'))->result_array();
        $listGudangKe     = $this->db->get_where('warehouse',array('desc'=>'subgudang'))->result_array();
    ...
    public function server_side_request_produksi(){
      $this->subgudang_request_material_model->server_side_request_produksi();
    }
    ...
    public function data_side_spk_material(){
  		$this->subgudang_request_material_model->data_side_spk_material();
  	}
