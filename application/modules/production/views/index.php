		...
		<div class="form-group row">
			...
			<div class="col-md-2">
				<select name='sales_order' id='sales_order' class='form-control input-sm chosen-select'>
					...
					<!-- <option value='1'>Waiting Input Mixing</option> -->
					<option value='2'>Waiting Input</option>
...
<div class="modal modal-default fade" id="dialog-popup" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<script type="text/javascript">
		...
		$(document).on('click', '.request', function(e) {
			...
			swal({
				...
				function(isConfirm) {
					if (isConfirm) {
						// loading_spinner();
						var baseurl = base_url + active_controller + '/release_aktual';
						$.ajax({
							url: baseurl,
							type: "POST",
							data: {
							...
							success: function(data) {
								window.open(base_url + active_controller + 'input_produksi/' + data.id, '_self');
							},
							error: function() {
								swal({
									title: "Error Message !",
		...
		$(document).on('click', '.requestFtackle', function(e) {
			e.preventDefault();
			...
			swal({
					title: "Are you sure?",
					text: "Input Aktual Produksi F-Tackle !",
					type: "warning",
					showCancelButton: true,
					confirmButtonClass: "btn-danger",
					confirmButtonText: "Yes, Process it!",
				...
				function(isConfirm) {
					if (isConfirm) {
						...
						$.ajax({
							...
							dataType: 'json',
							success: function(data) {
								window.open(base_url + active_controller + 'input_produksi_ftackle/' + data.id, '_self');
							},
							error: function() {
								swal({
									title: "Error Message !",
