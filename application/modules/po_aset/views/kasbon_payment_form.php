<div class="nav-tabs-area">
    <div class="tab-content">
        <div class="tab-pane active" id="area">
            ...
            <?= form_open(base_url().'po_aset/save_payment_kasbon',array('id'=>'frm_data','name'=>'frm_data','role'=>'form','class'=>'form-horizontal')) ?>
                ...
                <div class="box-body">
					<div class="form-group ">
						...
						<label for="tgl_cair" class="col-sm-2 control-label">Tgl Pemberian Cash</label>
						<div class="col-sm-3">
							<div class="input-group">
								<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
								<input type="text" class="form-control tgl" id="tgl_cair" name="tgl_cair" value="<?php echo set_value('tgl_cair', isset($data->tgl_cair) ? $data->tgl_cair: date("Y-m-d")); ?>" <?php if ($data->status==2) { ?>style="background:white" readonly <?php } else { echo 'disabled'; }?>>
							</div>
						</div>
					</div>
				<?php if ($data->status==3) {?>
					<div class="form-group ">
						<label for="tgl_expense" class="col-sm-2 control-label">Tgl Report Expense</label>
						<div class="col-sm-3">
							<div class="input-group">
								<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
								<input type="text" class="form-control tgl" id="tgl_expense" name="tgl_expense" value="<?php echo set_value('tgl_expense', isset($data->tgl_expense) ? $data->tgl_expense: date("Y-m-d")); ?>" style="background:white" readonly>
							</div>
						</div>
					    <label for="nilai_aktual" class="col-sm-2 control-label">Nilai Aktual</label>
						<div class="col-sm-3">
							<div class="input-group">
							<span class="input-group-addon"><i class="fa fa-list"></i></span>
							<input type="text" class="form-control divide" id="nilai_aktual" name="nilai_aktual" value="<?=$data->nilai_aktual?>" >
							</div>
						</div>
					</div>
				<?php }?>
					<div class="form-group ">
						...
						<div class="col-sm-3">
							<div class="input-group">
								<?php
								$datsumber[0]	= 'Select An Option';
								if ($data->status==2) {
									$readonly='';
								}else{
									$readonly='readonly';
								}
								echo form_dropdown('sumber_dana',$datsumber, $data->sumber_dana, array('id'=>'sumber_dana','class'=>'form-control',$readonly=>$readonly));
								?>
							</div>
						</div>
						...
						<div class="col-sm-3">
							<div class="input-group">
							<span class="input-group-addon"><i class="fa fa-list"></i></span>
							<input type="text" class="form-control divide" id="nilai_kasbon" name="nilai_kasbon" value="<?=$data->nilai_kasbon?>" readonly tabindex="-1">
							</div>
						</div>
					</div>
					<hr>
...
<script type="text/javascript">
    ...
    $('#frm_data').on('submit', function(e){
		...
		<?php if ($data->status==2) { ?>
			if($("#sumber_dana").val()=='0') d_error='Sumber dana harus dipilih';
			if($("#tgl_cair").val()=='') d_error='Tanggal pemberian cash harus diisi';
		<?php } ?>
		<?php if ($data->status==3) { ?>
			if($("#nilai_aktual").val()<=0) d_error='Nilai aktual harus lebih besar dari 0';
			if($("#tgl_expense").val()=='') d_error='Tanggal expense harus diisi';
		<?php } ?>
		if(d_error!=''){
			alert(d_error);
		}else{
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
					$.ajax({
						url: siteurl+"po_aset/save_payment_kasbon", dataType : "json", type: 'POST', data: formdata,
						success: function(msg){
							if(msg['save']=='1'){
								swal({
									title: "Sukses!", text: "Data Berhasil Di Simpan", type: "success", timer: 1500, showConfirmButton: false
								});
								cancel();
								window.location.reload();
							} else {
								swal({
									title: "Gagal!", text: "Data Gagal Di Simpan", type: "error", timer: 1500, showConfirmButton: false
								});
							};
							console.log(msg);
						},
						error: function(msg){
							swal({
								title: "Gagal!", text: "Ajax Data Gagal Di Proses", type: "error", timer: 1500, showConfirmButton: false
							});
							console.log(msg);
						}
					});
