...
class Barcode_cutting_model extends BF_Model{
  ...
  public function data_side_outstanding_qc(){
    ...
    foreach($query->result_array() as $row){
      // if($row['qty_qc'] > 0){
        ...
        $approve	= "<button type='button' data-id='".$row['id']."' data-id_cut_plan='".$row['id_cut_plan']."' class='btn btn-sm btn-default detail' title='QR Code' data-role='qtip'><i class='fa fa-qrcode fa-2x'></i></a>";
  ...
  public function get_query_outstanding_qc($product, $costcenter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
            ...
            FROM
                ...
                (SELECT @row:=0) r
            WHERE y.status != 'N' ".$costcenter_where." ".$product_where." AND (
