...
class Outgoing_stok_atk extends Admin_Controller
{
  //Permission
  protected $viewPermission   = 'Outgoing_Stok_ATK.View';
  protected $addPermission    = 'Outgoing_Stok_ATK.Add';
  ...
  public function __construct()
  {
    ...
    $this->load->model(array(
      'Outgoing_stok_atk/outgoing_stok_atk_model'
    ));
  ...
  public function index()
  {
    ...
    history("View data outgoing atk / household");
    $this->template->title('Gudang Stok / Outgoing ATK Household');
    $this->template->render('index');
  }

  public function data_side_request_material()
  {
    $this->outgoing_stok_atk_model->data_side_request_material();
  }
  ...
  public function request($id = null)
  {
    if ($this->input->post()) {
      ...
      if (!empty($data['detail'])) {
        foreach ($detail as $val => $valx) {
          ...
          if ($qty_packing > 0) {
            ...
            $qty_stock = 0;
            $get_qty_stock = $this->db->get_where('warehouse_stock', ['id_material' => $valx['id'], 'id_gudang' => 21])->row();
      ...
      if (!empty($ArrInsertDetail)) {
        ...
        foreach ($get_warehouse_detail as $item_detail) {
          ...
          $insert_costbook = $this->db->insert('tr_cost_book', [
            ...
            'nm_gudang_dari' => 'GUDANG ATK/HOUSEHOLD',
          ...
          $insert_costbook1 = $this->db->insert('tr_cost_book', [
            ...
            'tgl' => date('Y-m-d'),
            'no_transaksi' => $kode_trans,
            'jenis_transaksi' => 'In dari Gd. ATK/Household',
            'qty_transaksi' => $item_detail->qty_oke,
      ...
      if ($this->db->trans_status() === FALSE || $valid_qty_stock < 1) {
        ...
        move_warehouse_stok($ArrStock, 21, $id_costcenter, $kode_trans, null);
      ...
      $listGudang     = $this->db->get_where('warehouse', array('desc' => 'stok', 'id' => '20'))->result_array();
      ...
      $this->template->title('Outgoing ATK Household');
  ...
  {
    $this->outgoing_stok_atk_model->server_side_request_produksi();
  }
  ...
  {
    $this->outgoing_stok_atk_model->data_side_spk_material();
  }
  ...
  public function kurang_stock()
  {
    $kode_trans = $this->input->post('kode_trans');

    $get_data = $this->db->get_where('warehouse_adjustment', ['kode_trans' => $kode_trans])->row();

    $this->db->trans_start();
    $get_adjustment_detail = $this->db->get_where('warehouse_adjustment_detail', ['kode_trans' => $kode_trans])->result();
    foreach ($get_adjustment_detail as $item) {
      $get_stock = $this->db->get_where('warehouse_stock', ['id_material' => $item->id_material, 'id_gudang' => 21])->row();
      $stock_awal = $get_stock->qty_stock;
      $stock_pengurang = $item->qty_oke;

      $nilai_hasil = ($stock_awal - $stock_pengurang);

      $update_stock = $this->db->update('warehouse_stock', ['qty_stock' => $nilai_hasil], ['id' => $get_stock->id]);
      if (!$update_stock) {
        print_r($this->db->error($update_stock));
        exit;
      } else {
        $get_accessories = $this->db->get_where('accessories', ['id' => $item->id_material])->row();
        $get_gudang_dari = $this->db->get_where('warehouse', ['id' => $get_data->id_gudang_dari])->row();
        $get_gudang_ke = $this->db->get_where('warehouse', ['id' => $get_data->id_gudang_ke])->row();

        $insert_history = $this->db->insert('warehouse_history', [
          'id_material' => $item->id_material,
          'nm_material' => $get_accessories->stock_name,
          'id_gudang' => $get_data->id_gudang_dari,
          'kd_gudang' => $get_gudang_dari->kd_gudang,
          'id_gudang_dari' => $get_data->id_gudang_dari,
          'kd_gudang_dari' => $get_gudang_dari->kd_gudang,
          'id_gudang_ke' => $get_data->id_gudang_ke,
          'kd_gudang_ke' => $get_gudang_ke->kd_gudang,
          'qty_stock_awal' => $stock_awal,
          'qty_stock_akhir' => $nilai_hasil,
          'no_ipp' => $kode_trans,
          'jumlah_mat' => $stock_pengurang,
