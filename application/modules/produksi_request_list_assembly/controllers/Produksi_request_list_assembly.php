...
class Produksi_request_list_assembly extends Admin_Controller
{
    //Permission
    protected $viewPermission 	= 'Request_List_Produksi_Assembly.View';
    protected $addPermission  	= 'Request_List_Produksi_Assembly.Add';
    protected $managePermission = 'Request_List_Produksi_Assembly.Manage';
    protected $deletePermission = 'Request_List_Produksi_Assembly.Delete';
   ...
   public function __construct()
    {
        ...
        $this->load->model(array('Produksi_request_list_assembly/produksi_request_list_assembly_model'
                                ));
    ...
    public function index(){
      ...
      history("View data gudang produksi request assembly");
      $this->template->title('Gudang Material / Gudang Produksi / Request List Assembly');
      $this->template->render('index');
    }

    public function data_side_spk_material(){
  		$this->produksi_request_list_assembly_model->data_side_spk_material();
  	}
    ...
    public function request($id=null){
      if($this->input->post()){
        ...
        if(!empty($data['detail'])){
          foreach($detail AS $val => $valx){
            ...
            if($qty_packing > 0){
              ...
              if(!empty($valx['id'])){
                $getRequest = $this->db->get_where('so_spk_assembly_material',array('id'=>$valx['id']))->result_array();
        ...
        $ArrInsert = array(
          ...
          'no_ipp' 		      => $id_spk,
          'category' 			  => 'request produksi assembly',
          'jumlah_mat' 		      => $SUM_MAT,
        ...
        $this->db->trans_start();
          ...
          if(!empty($ArrInsertMaterial)){
            $this->db->insert_batch('so_spk_assembly_material',$ArrInsertMaterial);
          }
        $this->db->trans_complete();
        ...
        if($this->db->trans_status() === FALSE){
          ...
          history("Request assembly produksi to subgudang : ".$kode_trans);
        }
        echo json_encode($Arr_Data);
      }
      else{

        $listSO         = $this->db->select('a.*')->join('new_inventory_4 b','a.code_lv4=b.code_lv4')->get_where('so_spk_assembly a',array('a.deleted_date'=>NULL))->result_array();
        ...
        $this->template->title('Request Material Assembly');
        $this->template->render('request', $data);
      }
  	}

    public function server_side_request_produksi(){
      $this->produksi_request_list_assembly_model->server_side_request_produksi();
    }

    public function data_side_request_material(){
  		$this->produksi_request_list_assembly_model->data_side_request_material();
  	}
    ...
    public function print_spk_request(){
      ...
      $getData = $this->db
                            ...
                            ->join('bom_header c','c.no_bom=b.no_bom','left')
    ...
    public function get_list_spk(){
      ...
      $result	= $this->db->where_not_in('Y')->get_where('so_spk_assembly',array('id'=>$id_so))->result_array();

      if(!empty($result)){
        $option	= "<option value='0'>Select No. SPK</option>";
        foreach($result AS $val => $valx){
          $option .= "<option value='".$valx['id']."' data-kode_det='".$valx['kode_hub']."' data-qty='".$valx['propose']."' data-tanggal='".date('d-M-Y',strtotime($valx['due_date']))."'>".strtoupper($valx['no_spk'])."</option>";
        }
      }
      else{
    ...
    public function get_estimasi_material(){
      ...
      $result	= $this->db
                      ->select('a.id, a.category, a.weight, a.code_material, b.nama AS nm_material, b.code AS kode_material, b.code_lv4')
                      ->join('new_inventory_4 b','a.code_material=b.code_lv4','left')
                      ->like('a.kode_hub',$kode_det)
                      ->order_by('a.id','asc')
                      ->get('so_spk_assembly_material a')->result_array();
      ...
      $QUERY = "SELECT
                ...
                WHERE
                  b.no_ipp = '$id' 
                  AND category = 'request produksi assembly' 
      ...
      $ArrStokSPK[$value['id_uniq']] = $value['qty_req'];
      $ArrStokSPKAct[$value['id_uniq']] = $value['qty'];
      }
      ...
      if(!empty($result)){
        foreach($result AS $val => $valx){ $val++;
          ...
          $request = (!empty($ArrStokSPK[$id_uniq]))?$ArrStokSPK[$id_uniq]:0;
          $aktual = (!empty($ArrStokSPKAct[$id_uniq]))?$ArrStokSPKAct[$id_uniq]:0;
          ...
          $category = '-';
          if($valx['category'] == 'material flat sheet'){
            $category = 'Flat Sheet';
          }
          if($valx['category'] == 'material end plate'){
            $category = 'End Plate / Kick Plate';
          }
          if($valx['category'] == 'material ukuran jadi'){
            $category = 'Chequered Plate';
          }
          if($valx['category'] == 'material others'){
            $category = 'Others';
          }
