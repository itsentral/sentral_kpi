...
class Produksi_request_list_assembly_model extends BF_Model{

  public function __construct(){
    parent::__construct();

    $this->ENABLE_ADD     = has_permission('Request_List_Produksi_Assembly.Add');
		$this->ENABLE_MANAGE  = has_permission('Request_List_Produksi_Assembly.Manage');
		$this->ENABLE_VIEW    = has_permission('Request_List_Produksi_Assembly.View');
		$this->ENABLE_DELETE  = has_permission('Request_List_Produksi_Assembly.Delete');
  }
  ...
  public function data_side_request_material(){
    ...
    foreach($query->result_array() as $row){
      ...
      if($row['sts_confirm'] == 'N'){
        $print	= "&nbsp;<a href='".base_url('produksi_request_list_assembly/print_spk_request/'.$row['kode_trans'])."' target='_blank' class='btn btn-sm btn-info' title='Print SPK Permintaan Material' data-role='qtip'><i class='fa fa-print'></i></a>";
      }
  ...
  public function get_query_json_request_material($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              LEFT JOIN so_spk_assembly x ON a.no_ipp = x.id
              LEFT JOIN ms_costcenter c ON a.kd_gudang_ke=c.id_costcenter
              LEFT JOIN warehouse d ON a.id_gudang_dari=d.id
              LEFT JOIN bom_header e ON x.no_bom=e.no_bom
              LEFT JOIN new_inventory_4 z ON e.id_product = z.code_lv4,
              (SELECT @row:=0) r
            WHERE a.deleted_date IS NULL AND a.category='request produksi assembly' AND (
