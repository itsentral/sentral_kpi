...
class Costbook_material extends Admin_Controller
{

	//Permission

	protected $viewPermission   = "Costbook_Material_Per_Item.View";
	protected $addPermission    = "Costbook_Material_Per_Item.Add";
	protected $managePermission = "Costbook_Material_Per_Item.Manage";
	protected $deletePermission = "Costbook_Material_Per_Item.Delete";

	protected $viewPermission2   = "Costbook_Material_Neraca_Gudang.View";
	protected $addPermission2    = "Costbook_Material_Neraca_Gudang.Add";
	protected $managePermission2 = "Costbook_Material_Neraca_Gudang.Manage";
	protected $deletePermission2 = "Costbook_Material_Neraca_Gudang.Delete";

	public function __construct()
	{
		parent::__construct();
		$this->load->library(array('Mpdf', 'upload', 'Image_lib'));
		$this->load->model('Costbook_material/Costbook_material_model');
		$this->template->title('Costbook Material Per Item');
	...
	public function per_item()
	{
		...
		$this->db->select('a.qty_stock, a.id_material, a.nm_material, b.code as kode_produk, c.id AS id_gudang, c.nm_gudang');
		...
		$this->db->where('b.category', 'material');
	...
	public function neraca_gudang()
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Costbook Material Neraca Gudang');
	...
	public function view_per_item()
	{
		...
		$get_nm_material = $this->db->select('a.nama as nm_material')
	...
	public function get_costbook_data()
	{
		...
		$this->db->where('a.tipe_material', 'material');
		$this->db->where('DATE_FORMAT(a.tgl, "%m") =', sprintf('%02s', $periode_bulan));
		$this->db->where('DATE_FORMAT(a.tgl, "%Y") =', $periode_tahun);
		...
		$get_data = $this->db->query('SELECT a.jenis_transaksi, a.no_transaksi, a.tgl, a.value_transaksi, IF(a.jenis_transaksi LIKE "%Out%", "Out", "In") as tipe_transaksi, b.nm_gudang as nm_gudang_dari, c.nm_gudang as nm_gudang_ke FROM tr_cost_book a LEFT JOIN warehouse b ON b.id = a.id_gudang_dari LEFT JOIN warehouse c ON c.id = a.id_gudang_ke WHERE a.tipe_material = "material" AND DATE_FORMAT(a.tgl, "%m") = "'.sprintf('%02s', $periode_bulan).'" AND DATE_FORMAT(a.tgl, "%Y") = "'.$periode_tahun.'" AND a.jenis_transaksi NOT LIKE "%saldo%" '.$where_warehouse.' ORDER BY a.created_on ASC')->result();
		// if(!$get_data) {
