...
class Gudang_wip_ftackle_model extends BF_Model{
  ...
  public function get_query_json_gudang_wip($sales_order, $date_filter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              (SELECT @row:=0) r
            WHERE a.deleted_date IS NULL ".$WHERE2." ".$sales_date_filter." ".$sales_order_where." AND y.code_lv1 = 'P123000009' AND (
  ...
  public function data_side_gudang_wip_inout(){
    ...
    $GET_LEVEL4 = get_inventory_lv4();
    $GET_LEVEL2 = get_inventory_lv2();
    $tgl_awal = (!empty($requestData['tgl_awal']))?$requestData['tgl_awal']:date('Y-m-d');
    $tgl_akhir = (!empty($requestData['tgl_akhir']))?$requestData['tgl_akhir']:date('Y-m-d');
    foreach($query->result_array() as $row){
      $total_data     = $totalData;
      $start_dari     = $requestData['start'];
      $asc_desc       = $requestData['order'][0]['dir'];
      ...
      $countIN  = $this->db->get_where('so_internal_product',array('id_key_spk'=>$row['id_key_spk'],'DATE(close_date) >='=>$tgl_awal,'DATE(close_date) <='=>$tgl_akhir))->result_array();
      $countOUT = $this->db->get_where('so_internal_product',array('id_key_spk'=>$row['id_key_spk'],'DATE(qc_date) >='=>$tgl_awal,'DATE(qc_date) <='=>$tgl_akhir))->result_array();
