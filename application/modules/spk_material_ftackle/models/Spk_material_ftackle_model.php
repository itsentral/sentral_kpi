...
class Spk_material_ftackle_model extends BF_Model{

  public function __construct(){
    parent::__construct();

    $this->ENABLE_ADD     = has_permission('SPK_Produksi_F-Tackle.Add');
		$this->ENABLE_MANAGE  = has_permission('SPK_Produksi_F-Tackle.Manage');
		$this->ENABLE_VIEW    = has_permission('SPK_Produksi_F-Tackle.View');
		$this->ENABLE_DELETE  = has_permission('SPK_Produksi_F-Tackle.Delete');
  }
  ...
  public function get_query_json_spk_material($sales_order, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              (SELECT @row:=0) r
            WHERE a.deleted_date IS NULL AND z.code_lv1 = 'P123000009' ".$sales_order_where." AND (
  ...
  public function data_side_spk_reprint(){
    ...
    foreach($query->result_array() as $row){
      ...
      $print	= "<a href='".base_url('spk_material_ftackle/print_spk/'.$row['kode'])."' target='_blank' title='Print SPK' data-role='qtip'>Print</a>";
  ...
  public function get_query_json_spk_reprint($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              LEFT JOIN bom_header d ON a.no_bom=d.no_bom,
              (SELECT @row:=0) r
            WHERE a.deleted_date IS NULL AND z.code_lv1 = 'P123000009' AND (
              a.code_lv4 LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR z.nama LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR d.variant_product LIKE '%".$this->db->escape_like_str($like_value)."%'
              OR d.color LIKE '%".$this->db->escape_like_str($like_value)."%'
