...
class Gudang_wip_cutting_model extends BF_Model{
  ...
  public function data_side_gudang_wip(){
    ...
    foreach($query->result_array() as $row){
      ...
      $nestedData[]	= "<div align='left'>".number_format($row['length'],2)." x ".number_format($row['width'],2)."</div>";
  ...
  public function get_query_json_gudang_wip($sales_order, $date_filter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
    ...
    $sales_date_filter = "";
    $table = 'so_spk_cutting_product';
    $WHERE2 = "AND b.qc_date IS NULL";

    $key1 = "b.kode_det";
    $field1 = "COUNT(b.id) AS qty";
    $group1 = " GROUP BY b.kode_det, b.id_key_spk";
    ...
    $sql = "SELECT
            ...
            FROM
              ".$table." b
              LEFT JOIN so_spk_cutting_request z ON $key1 = z.id
              LEFT JOIN so_spk_cutting a ON a.id=z.id_so AND z.status_id = '1'
              LEFT JOIN so_spk_cutting_plan p ON b.id_key_spk = p.id
