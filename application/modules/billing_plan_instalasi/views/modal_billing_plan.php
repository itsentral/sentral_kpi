...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        $nilai_ppn = ($total_all * $results->ppn / 100);

        $total = ($total_all + $nilai_ppn);
        ?>
    </tbody>
    <tbody>
        <tr>
        ...
        <tr>
            ...
            <td class="text-right"><?= '(' . $currency . ') ' . number_format($nilai_ppn, 2) ?></td>
        </tr>
        <tr>
...
<h4>
    ...
    <button type="button" class="btn btn-sm btn-success add_detail_invoice">
        <i class="fa fa-plus"></i> Add Billing Type
    </button>
    <input type="hidden" class="no_detail" value="1">
</h4>
...
<table class="table table-bordered">
    ...
    <tbody class="list_detail_invoice">

    </tbody>
</table>


<input type="hidden" class="total_all" value="<?= $total ?>">
...
<script type="text/javascript">
    ...
    $(document).on('click', '.add_detail_invoice', function() {
        var no_detail = parseFloat($('.no_detail').val());

        var hasil = '<tr class="col_detail_inv_' + no_detail + '">';
        hasil += '<td class="text-center">';
        hasil += '<select name="detail[' + no_detail + '][tipe_billing_plan]" class="form-control form-control-sm">';
        hasil += '<option value="">- Billing Type -</option>';
        hasil += '<option value="1">DP</option>';
        hasil += '<option value="2">Progress</option>';
        hasil += '<option value="3">Retensi</option>';
        hasil += '<option value="4">Jaminan</option>';
        hasil += '</select>';
        hasil += '</td>';
        hasil += '<td>';
        hasil += '<input type="text" class="form-control form-control-sm remarks_invoice_' + no_detail + '" name="detail[' + no_detail + '][remarks]" data-no="' + no_detail + '">';
        hasil += '</td>';
        hasil += '<td>';
        hasil += '<input type="text" class="form-control form-control-sm auto_num text-right hitung_nilai_by_persen persen_invoice_' + no_detail + '" name="detail[' + no_detail + '][persen_invoice]" data-no="' + no_detail + '">';
        hasil += '</td>';
        hasil += '<td>';
        hasil += '<input type="text" class="form-control form-control-sm auto_num text-right hitung_persen_by_nilai value_invoice_' + no_detail + '" name="detail[' + no_detail + '][value_invoice]" data-no="' + no_detail + '">';
        hasil += '</td>';
        hasil += '<td>';
        hasil += '<input type="date" class="form-control form-control-sm" name="detail[' + no_detail + '][billing_plan_date]">';
        hasil += '</td>';
        hasil += '<td>';
        hasil += '<button type="button" class="btn btn-sm btn-danger del_invoice_detail" data-no="' + no_detail + '">';
        hasil += '<i class="fa fa-trash"></i>';
        hasil += '</button>';
        hasil += '</td>';
        hasil += '</tr>';

        $('.list_detail_invoice').append(hasil);
        no_detail = parseFloat(no_detail + 1);
        $('.no_detail').val(no_detail);

        loadmod();
    });

    $(document).on('change', '.hitung_nilai_by_persen', function() {
        var no = $(this).data('no');
        ...
        var nilai_invoice = parseFloat(total_all * persen / 100);

        $('.value_invoice_' + no).val(nilai_invoice.toLocaleString('en-US', {
            maximumFractionDigits: 2
        }));
    });

    $(document).on('change', '.hitung_persen_by_nilai', function() {
        var no = $(this).data('no');
        ...
        var persen_invoice = parseFloat(nilai / total_all * 100);

        $('.persen_invoice_' + no).val(persen_invoice.toLocaleString('en-US', {
            maximumFractionDigits: 2
        }));
    });

    $(document).on('click', '.del_invoice_detail', function() {
        var no_detail = $(this).data('no');

        $('.col_detail_inv_' + no_detail).remove();
    });
</script>