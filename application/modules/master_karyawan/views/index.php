...
<div class="nav-tabs-supplier">
  <ul class="nav nav-tabs">
    <li class="active"><a href="#Karyawan" data-toggle="tab" aria-expanded="true">Karyawan</a></li>
    <li class=""><a href="#Departeman" data-toggle="tab" aria-expanded="false">Departeman</a></li>
  </ul>
</div>
...
<div class="box">
	...
	<div class="box-body">
		...
		<tr>
			...
			<th>NEmployee Name</th>
			<th>Pleace And Date Of Born</th>
			<th>Gender</th>
		...
		<tr>
			...
			<?php if($ENABLE_MANAGE) : ?>
				<a class="btn btn-success btn-sm edit" href="javascript:void(0)" title="Edit" data-id_karyawan="<?=$record->id_karyawan?>"><i class="fa fa-edit"></i>
				</a>
			<?php endif; ?>
    ...
    <div class="tab-pane" id="Departeman">
<div class="box">
	<div class="box-header">
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-success btn-sm tambah" href="javascript:void(0)" title="Tambah"><i class="fa fa-plus">&nbsp;</i>Tbh</a>
			<?php endif; ?>
	...
	<div class="box-body">
		...
		<tr>
			<th width="5">#</th>
			<th width="13%">Departemant ID</th>
			<th>Departemant Name</th>
			<?php if($ENABLE_MANAGE) : ?>
			<th width="13%">Action</th>
			<?php endif; ?>
		</tr>
		...
		<?php if(empty($results)){
			...
			$numb=0; foreach($results2 AS $records2){ $numb++; ?>
		<tr>
		    <td><?= $numb; ?></td>
			<td><?= $records2->id_divisi ?></td>
			<td><?= $records2->nm_divisi ?></td>
			<td style="padding-left:20px">
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-primary btn-sm viewdep" href="javascript:void(0)" title="Viewdep" data-id_divisi="<?=$records2->id_divisi?>"><i class="fa fa-eye"></i>
				</a>
			<?php endif; ?>

			<?php if($ENABLE_MANAGE) : ?>
				<a class="btn btn-success btn-sm editdep" href="javascript:void(0)" title="Editdep" data-id_divisi="<?=$records2->id_divisi?>"><i class="fa fa-edit"></i>
				</a>
			<?php endif; ?>

			<?php if($ENABLE_DELETE) : ?>
				<a class="btn btn-danger btn-sm deletedep" href="javascript:void(0)" title="Deletedep" data-id_divisi="<?=$records2->id_divisi?>"><i class="fa fa-trash"></i>
				</a>
			<?php endif; ?>
			</td>
...
<script type="text/javascript">
	...
	$(document).on('click', '.view', function(){
		...
		$.ajax({
			type:'POST',
			url:siteurl+'master_karyawan/addDepartement',
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
