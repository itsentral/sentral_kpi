	...
	public function get_data_where_array($table,$where){
  	...
  	public function get_json_bom(){
		...
		foreach($query->result_array() as $row){
			...
			$nestedData[]	= "<div align='left'>".strtoupper(strtolower($row['color']))."</div>";
			$moq = (!empty($row['moq']))?number_format($row['moq'],4):'-';
			$nestedData[]	= "<div align='right'>".$moq."</div>";

        	$berat = $this->db->query("SELECT SUM(weight) AS weight FROM bom_detail WHERE no_bom='".$row['no_bom']."'")->result();
			$nestedData[]	= "<div align='right'>".number_format($berat[0]->weight,4)."</div>";
			
			$waste_product = (!empty($row['waste_product']))?number_format($row['waste_product'],2):'-';
			$waste_setting_resin = (!empty($row['waste_setting_resin']))?number_format($row['waste_setting_resin'],4):'-';
			$waste_setting_glass = (!empty($row['waste_setting_glass']))?number_format($row['waste_setting_glass'],4):'-';
			$nestedData[]	= "<div align='right'>".$waste_product."</div>";
			$nestedData[]	= "<div align='right'>".$waste_setting_resin."</div>";
			$nestedData[]	= "<div align='right'>".$waste_setting_glass."</div>";
	...
	public function get_query_json_bom($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		$sql = "SELECT
				...
				FROM
					...
					(SELECT @row:=0) r
				WHERE 1=1 AND a.deleted_date IS NULL AND a.category = 'standard' AND 
					(
